...
import MenuLight from "./Images/menu-light.png";
...
const Navbar = () => {
  ...
  const list = () => (
    ...
    >
      <List>
        {[
          { text: "Hem", icon: <HomeRoundedIcon />, link: "/" },
          { text: "Om Mig", icon: <AccountBoxRoundedIcon />, link: "/OmMig" },
          {
            text: "Portfolio",
            icon: <ArticleRoundedIcon />,
            link: "/Portfolio",
          },
          { text: "Blogg", icon: <NewspaperRoundedIcon />, link: "/Blogg" },
        ].map((item, index) => (
          <ListItem key={index} disablePadding>
            <Link
              to={item.link}
              className={`${styles.link} ${
                darkMode ? styles.darkLink : styles.lightLink
              }`}
            >
    ...
    <>
      <nav className={`${styles.nav}`}>
        ...
        <div className="hamburger-menu" onClick={handleToggleMenu}>
          <img src={darkMode ? MenuDark : MenuLight} alt="Menu" />
        </div>
        ...
        {/* Byt mellan mörkt och ljust läge */}
